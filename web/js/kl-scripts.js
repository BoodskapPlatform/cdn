...
var KallyasConfig = {
	...
	searchDomain: 'https://boodskap.io',
...
// Sticky Header
	if(KallyasConfig.enableStickyHeader && ! KallyasConfig.enableChaserMenu){
		...
		$(window).scroll(function(){
			...
			if(scrolled > 1){
				$body.addClass('sticky-header');
				header.addClass('header--is-sticked').removeClass('header--no-sticked');
			}
			else if (scrolled < 0 ) {
				$body.removeClass('sticky-header');
				header.removeClass('header--is-sticked').addClass('header--no-sticked');
				...
				$body.removeClass('sticky-header');
				header.removeClass('header--is-sticked');
			}
			else {
				header.addClass('header--is-sticked').removeClass('header--no-sticked');
			}
...
// Responsive Main-Menu
	...
	var start_responsive_menu = function(){
		...
		responsive_menu.find('li:has(> ul)').addClass('zn_res_has_submenu').prepend('');
		// ADD BACK BUTTONS
	...
	$( window ).on( 'load resize' , function(){
	   triggerMenu();
		...
		if ( $(window).width() < 1200 ) {
			...
			if(closeMenuSender){
				closeMenuSender.on('clzn_res_back_icon fas fa-chevron-leftick', function(e){
					e.preventDefault();
					e.stopPropagation();
...
// * Use this as your ID below instead!
	...
	if(twFeed.length > 0 && typeof(twitterFetcher) != 'undefined') {
		...
		var twitter_config = {
			"profile": {"screenName": 'boodskapIOT'},
			"domId": '',
			"maxTweets": 1,
			"enableLinks": true,
			"showUser": false,
